...
bool flag_lmb_win_active=false; // ЛКМ удерживается и Win нажат
bool flag_lmb_esc_active=false; // ЛКМ удерживается и Esc нажат
...
case WM_RBUTTONUP:
			...
			case WM_LBUTTONDOWN:
				...
				// для отрисовки красных квадратиков
				InvalidateRect(MHhwnd,NULL,TRUE);
				// AHK ЛКМ=Win: запускаем таймер на 3 секунды
				if(MHSettings::flag_lmb_win_ahk && !flag_lmb_win_active)
					SetTimer(MHhwnd, 7, 3000, NULL);
				// ЛКМ=Esc: запускаем таймер на 3 секунды
				if(MHSettings::flag_lmb_esc && !flag_lmb_esc_active)
					SetTimer(MHhwnd, 8, 3000, NULL);
				// Проверим, не собираемся ли мы ткнуть в наш квадратик?
				// Что означает начало скролла
			...
			case WM_LBUTTONUP:
				...
				// для отрисовки красных квадратиков
				InvalidateRect(MHhwnd,NULL,TRUE);
				// AHK ЛКМ=Win: отпускаем Win если был нажат, убиваем таймер
				if(MHSettings::flag_lmb_win_ahk) {
					KillTimer(MHhwnd, 7);
				...
				// ЛКМ=Esc: отпускаем Esc если был нажат, убиваем таймер
				if(MHSettings::flag_lmb_esc) {
					KillTimer(MHhwnd, 8);
	...
	return CallNextHookEx(handle,disabled,wParam,lParam);
}